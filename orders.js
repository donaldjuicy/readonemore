...
router.post('/', authenticate, async (req, res) => {
  ...
  try {
    const { delivery_method, address_id, notes } = req.body;
    ...
    const bookCount = items.reduce((s, i) => s + i.quantity, 0);
    const shippingFee = (delivery_method === 'pickup' || bookCount >= +process.env.FREE_SHIPPING_MIN_BOOKS)
      ? 0 : +process.env.SHIPPING_FEE_DKK;
    const total = subtotal + shippingFee;

    // 4. Create order
    const orderResult = await client.query(
      `INSERT INTO orders (user_id, delivery_method, address_id, subtotal, shipping_fee, total, notes)
       VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *`,
      [req.user.userId, delivery_method || 'shipping', address_id || null,
       subtotal.toFixed(2), shippingFee.toFixed(2), total.toFixed(2), notes || null]
    );
    ...
    // 7. Evaluate badges asynchronously (mirrors AchievementEvaluationScheduler)
    evaluateBadges(req.user.userId).catch(console.error);
